...
- [ ] **macOS VPIO excludes Bluetooth combo headsets** — With AirPods (or another Bluetooth combo headset) selected as the default input and `macos_input_vpio_enabled` ON, join a Meet/Zoom call. The conferencing app's mic must stay live and selectable, screenpipe must capture it through HAL with non-zero audio, and logs must contain `macOS VPIO requested but using HAL` with the Bluetooth-combo reason — never `enabling VoiceProcessingIO (AEC)` for that device. Outside meetings, the existing Bluetooth mic gate must still keep the headset in A2DP. (#6213)
